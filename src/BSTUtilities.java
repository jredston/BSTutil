...
public class BSTUtilities {
    ...
    /**
     * A no argument constructor that should initialize an empty BST. Fill out
     * this constructor USING CONSTRUCTOR CHAINING. (Do not leave it blank.)
     */
    public BSTUtilities() {
        this(new int[0]);
     ...
     *
     * @param nums
     *            The integers to add to the tree. This will never be null.
     */
     ...
     *            The integers to add to the tree. This will never be null.
     * @param end
     *            The index to stop adding items from the array.
     ...
     *            The integers to add to the tree. This will never be null.
     * @param start
     *            The index to start adding items from.
     * @param end
     *            The index to stop adding items.
     * @throws ArrayIndexOutOfBoundsException
     *             if start < 0 or end > nums.length.
     ...
     * @param current
     *            the BSTNode in the tree currently under examination
     ...
     *
     * @param current
     *            The current node for this subtree.
     ...
     *
     * @param current
     *            The current node you're looking at.
    ...
    private int size(BSTNode current) {
        ...
        return 1 + size(current.getRight()) + size(current.getLeft());
    }
    ...
    /**
     * Multiplies the left and right subtrees, according to a formula defined in
     * the helper method's Javadoc comment.
     *
     * This method can and should be done in one line only.
     *
     ...
     * The formula:
     *
     * current node's data + 6 * baseMultiply(left subtree) + 7 *
     * baseMultiply(right subtree)
     *
     * @param current
     *            The current node you're looking at.
    ...
    private int baseMultiply(BSTNode current) {
        ...
        return current.getData() + 6 * baseMultiply(current.getLeft()) + 7
                * baseMultiply(current.getRight());
     ...
     * passed in.
     *
     * HINT: For this method to be efficient, remember that the data in a BST is
     * ordered in a certain way. You may not have to traverse an entire subtree.
     *
     * This method can and should be done in one line only.
     *
     * @param data
     *            Count the number of items greater than this element.
     ...
     *
     * This is the helper method to be used for {@code thresholdCount(int data)}
     * .
     *
     * @param current
     *            The current node you're looking at.
    ...
    private int thresholdCount(BSTNode current, int data) {
        ...
        if (current.getData() > data) {
            // increase and add right and left sub tree
    ...
    /**
     * Calculates the sum of the nodes only on even levels of the tree. The root
     * is considered to be level 0, (0 is even) and the "number" of the level
     * increases as you go farther down the tree.
     *
     * For example, for a tree that looks like the following: 8 / \ / \ 6 10 / \
     * \ / \ \ 4 7 15
     *
     ...
     * This method can and should be done in one line only.
     *
     * @return the sum of the data contained in the nodes only on even levels of
     *         the tree.
     */
     ...
     *
     * @param current
     *            The current node you are looking at.
     ...
     *
     * @return smallest item in the tree. If the tree is empty, return
     *         {@code null}.
     */
     ...
     * This method can and should be done in five lines or less.
     *
     * @return largest item in the tree. If the tree is empty, return
     *         {@code null}.
     */
     ...
     *
     * The height of a {@code null} node is -1, and the height of a leaf node is
     * 0.
     *
     * @param current
     *            The current node you're looking at.
     ...
     * formatted as follows: [currentData, leftSubtree, rightSubtree]
     *
     * For example, for a tree that looks like the following: 8 / \ / \ 6 10 / \
     * \ / \ \ 4 7 15
     *
     * The string should be: [8, [6, [4, [], []], [7, [], []]], [10, [], [15,
     * [], []]]
     *
